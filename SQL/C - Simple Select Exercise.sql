...
SELECT  CourseId,                   -- This column is a non-aggregate
        AVG(Mark) AS 'Average Mark' -- This column performs Aggregate (produce 1 value) --a non'aggregate will always be in the group by (?)
FROM    Registration
...
HAVING AVG(Mark) > 80 --filter but only for Aggregate ones!!!
...
-- TODO: Student Answer Here....
SELECT ClubId, COUNT(StudentID) AS 'Studet in Clubs'
FROM Activity
GROUP BY ClubID
...
-- TODO: Student Answer Here....
SELECT ClubID, COUNT(StudentID) AS 'Club has over three students'
FROM Activity 
GROUP BY ClubID
...
SELECT COUNT(Gender) 
FROM Student
GROUP BY Gender='Female' and Gender='Male'
HAVING 
