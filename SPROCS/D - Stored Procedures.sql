...
--Try manually inserting a position with a name that is too long
INSERT INTO Position(PositionDescription)
VALUES ('The IT guy who has to figure out why things are not working as expected')
--errors from running the line above was generated by the position table
--the position table has declared the PositionDescription as a varchar (50)
--that means the database will not accept a description that is longer because
--attempting to do so would "lose" date (the string value would be "truncated" ).
--By the way, what's the highest positionID so far?
SELECT MAX (PositionID) FROM Position
--what will the PositionID be after doing the next insert?
EXEC AddPosition 'Database Administrator'
